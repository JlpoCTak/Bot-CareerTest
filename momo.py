...
def geology():
    meteorologia = 0
    hydrometeorological_observer = 0
    hydrologya = 0
    cartographer = 0
    print("В этом отделе вам доступы данные специальности:")
    print("Метеорология,\n Картография,\n Гидрология,\n Гидрометнаблюдатель")

    print("Как называется процесс испарения воды с поверхности Мирового океана и превращения его в водяной пар:")
    print("1. конденсация")
    print("2. испарение")
    print("3. круговорот воды")#*
    choice = input("Введите номер ответа: ")
    if choice == "1":
        geology2()
    elif choice == "2":
        ...
        hydrometeorological_observer += 1
        geology2()

def geology2():
    print(" Какой прибор используется для измерения температуры воздуха:")
    print("1. термометр") #*
    print("2. барометр")
    print("3. анемометр")
    choice = input("Введите номер ответа: ")
    if choice == "1":
        ...
        geology3()
def geology3():
    print("Какая из перечисленных картографических проекций является наиболее распространенной:")
    print("1. цилиндрическая")
    print("2. коническая")
    print("3. азимутальная")#*
    choice = input("Введите номер ответа: ")
    if choice == "1":
        geology4()
    elif choice == "2":
        ...
        geology4()
def geology4():
    print("Какие виды гидрологического оборудования используются для изучения водных объектов?")
    print("1. Гидрологические станции")#*
    print("2. Гидрологические тепловизионные камеры")
    print("3. Гидрологические дроны с камерами")
    choice = input("Введите номер ответа: ")
    if choice == "1":
        hydrologya += 1
        ()
        ...
        ()
max_geology = max(meteorologia, hydrometeorological_observer, hydrologya, cartographer)
        # 05.02.03:Метеорология
        # 05.02.01:Картография
        # 05.02.02:Гидрология
        # 05.01.01:Гидрометнаблюдатель
        # Как называется процесс испарения воды с поверхности Мирового океана и превращения его в водяной пар: а) конденсация б) испарение в) круговорот воды
        # Какой прибор используется для измерения температуры воздуха: а) термометр б) барометр в) анемометр
        # Какая из перечисленных картографических проекций является наиболее распространенной: а) цилиндрическая б) коническая в) азимутальная
        # Какие виды гидрологического оборудования используются для изучения водных объектов?
def hoz():
    print("вы предпочтёте:")
...
# 35.02.07:Механизация сельского хозяйстваа
...
# 35.01.07:Сушильщик в бумажном производстве
...
# Вопрос: Что является главной задачей мастера по лесному хозяйству?
# Варианты ответов:
#
# Планирование и контроль проведения лесохозяйственных работ (правильный ответ).
# Осуществление научных исследований в сфере лесного хозяйства.
# Организация и проведение лесозаготовительных работ.
# 2. Станочник деревообрабатывающих станков:
# Вопрос: Какой из перечисленных инструментов применяется для обработки деревянных заготовок на станках деревообрабатывающего типа?
# Фрезерный резец (правильный ответ).
# Токарные резцы.
# Сверла.
# 3. Станочник-обработчик:
# Вопрос: Какие из перечисленных операций выполняются на станках такого типа?
# Обработка поверхностей деталей (правильный ответ).
# Сборка и разборка разнообразных механизмов и устройств.
# Нарезка резьбы.
# 4. Оператор линий и установок в деревообработке:
# Вопрос: Какую функцию обычно выполняет оператор при работе с деревообрабатывающими линиями?
# Настройка оборудования (правильный ответ).
# Управление процессами обработки древесины.
# Контроль качества производимой продукции.
# 5. Контролёр качества материалов и продукции деревообрабатывающего производства:
# Продолжи
# Вопрос: Каким образом, как правило, контролер определяет уровень качества продукции на деревообрабатывающих предприятиях?
#
# Используя специальное измерительное оборудование (правильный ответ).
# Путем визуального осмотра изделий.
# Оператор машин по производству бумаги и картона:
# Вопрос: Какая функция НЕ выполняется оператором в процессе работы машины по производству бумаги и картона?
#
# Варианты ответов:
#
# Загрузка сырья
...
# Правильный ответ: Разгрузка готовой продукции
# 2. Сушильщик в бумажном производстве:
# Вопрос: С помощью какого оборудования сушильщик осуществляет свою работу? Варианты ответов: * Сушильные камеры
#
# Бумагоделательные машины
# Упаковочные станки
# Правильный ответ: Сушильные камеры
# Контролер целлюлозно-бумажного производства:
# Вопрос: В каких случаях контролер может использовать специальное измерительное оборудование? Варианты ответов: * При проверке качества готовой продукции
# При контроле параметров технологического процесса
# При осмотре оборудования
# Правильный ответ: При проверке качества готовой продукции
...
def ocean():
    ...
    print("2. Работает с различными видами оборудования, такими как рыборезки")
    print("3. Сохраняет водные биоресурсы и аквакультуру")
    if choice == "1":
        master +=1
        ocean2()
    elif choice == "2":
        ...
        ocean2()
#сделать систему которая за правильный ответ +1, за неправильный +0, если в неправильном ответе были задействлванны ответы свзанные с другой темой -1 по этой теме

def ocean2():
    print("Какими знаниями и навыками должен обладать обработчик рыбы и морепродуктов?")
    print("1. Обработка и переработка улова")
    print("2. Знание технологии обработки рыбы и морепродуктов")#*
    print("3. Обработчик рыбы и морепродуктов должен обладать знаниями в области кулинарии и навыками приготовления пищи.")
    if choice == "1":
        industry_fish -= 1
        ocean3()
    elif choice == "2":
        ...
        ocean3()
def ocean3():
    print("В чем особенности работы рыбака прибрежного лова?")
    print("1. Рыбак прибрежного лова работает на больших судах и ловит рыбу в открытом океане.")
    print("2. Работа на небольших судах или лодках, которые могут быть более маневренными и удобными для работы в прибрежных водах.")#*
    print("3. Рыбак прибрежного лова использует только удочку для ловли рыбы.")
    if choice == "1":
        ocean4()
    elif choice == "2":
        fishman += 1
        ...
        ocean4()
def ocean4():
    print("Какие дисциплины включает в себя ихтиология и рыбоводство?")
    print("1. Изучают рыбу в природе")
    print("2. Охрана водных биоресурсов и окружающей среды.")
    print("3. Изучение рыб, их классификация, экология, поведение и распределение, разведение и выращивание рыб в искусственных условиях")#*
    if choice == "1":
        ocean5()
    elif choice == "2":
        industry_fish -= 1
        ...
        ocean5()
def ocean5():
    print("В чем состоит процесс обработки водных биоресурсов?")
    print("1. Лов -> доставка -> очистка -> заморозка")
    print("2. Лов -> заморозка-> очистка -> доставка")
    print("3. Лов -> очистка -> заморозка -> доставка")#*
    ...
    if choice == "1":
        ...
        ocean6()
def ocean6():
    print("Какие аспекты включает в себя промышленное рыболовство?")
    print("1. Изучение рыб, их классификация, экология, поведение и распределение, разведение и выращивание рыб в искусственных условиях")
    print("2. Знание технологии обработки рыбы и морепродуктов")
    print("3. Лов рыбы и других водных биоресурсов")#*
    if choice == "1":
        ihteology -= 1
        ()
    elif choice == "2":
        ...
        industry_fish += 1
        ()
max = max(master, fish_handler, fishman, ihteology, bio, industry)
print(max)

            # 35.01.16:Мастер по водным биоресурсам и аквакультуре
            # 35.01.17:Обработчик рыбы и морепродуктов
            # 35.01.18:Рыбак прибрежного лова
            # 35.02.09:Ихтиология и рыбоводство
            # 35.02.10:Обработка водных биоресурсов
            # 35.02.11:Промышленное рыболовство
            
            # вопросы
            # 1 В чем состоят основные обязанности мастера по водным биоресурсам и аквакультуре?
            # 2 Какими знаниями и навыками должен обладать обработчик рыбы и морепродуктов?
            # 3 В чем особенности работы рыбака прибрежного лова?
            # 4Какие дисциплины включает в себя ихтиология и рыбоводство?
            # 5 В чем состоит процесс обработки водных биоресурсов?
            # 6 Какие аспекты включает в себя промышленное рыболовство?
...
# 18.01.04:Изготовитель изделий строительной керамики
...
# 18.01.07:Аппаратчик производства стекловолокнистых материалов и стеклопластиков
# 18.01.08:Мастер-изготовитель деталей и изделий из стекла
# 18.01.09:Мастер-обработчик стекла и стеклоизделий
# 18.01.10:Отдельщик и резчик стекла
# 18.01.11:Контролер стекольного производства
# 18.01.12:Изготовитель фарфоровых и фаянсовых изделий
# 18.01.13:Отделочник и комплектовщик фарфоровых и фаянсовых изделий
# 18.01.14:Контролер-приемщик фарфоровых, фаянсовых и керамических изделий
# 18.01.15:Изготовитель эмалированной посуды
# 18.01.16:Аппаратчик в производстве химических волокон
# 18.01.17:Оператор в производстве химических волокон
# 18.01.18:Аппаратчик производства синтетических смол и пластических масс
# 18.01.19:Машинист-оператор в производстве изделий из пластмасс
# 18.01.20:Прессовщик изделий из пластмасс
# 18.01.21:Машинист-аппаратчик подготовительных процессов в производстве резиновых смесей, резиновых технических изделий и шин
# 18.01.22:Оператор в производстве шин
# 18.01.23:Оператор процессов вулканизации
# 18.01.24:Мастер шиномонтажной мастерской
# 18.01.25:Оператор в производстве резиновых технических изделий и обуви
